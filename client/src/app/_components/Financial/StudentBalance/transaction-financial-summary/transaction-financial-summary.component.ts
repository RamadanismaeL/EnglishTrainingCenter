...
@Component({
  selector: 'app-transaction-financial-summary',
  imports: [
    AgGridAngular,
    MatIconModule,
    MatTooltipModule,
  ...
  templateUrl: './transaction-financial-summary.component.html',
  styleUrl: './transaction-financial-summary.component.scss'
})
export class TransactionFinancialSummaryComponent implements OnInit, OnDestroy {
  ...
  private studentFullName : string | undefined;

  constructor(private studentService: StudentsService, private studentShareId: StudentShareIdService, private notificationHub: NotificationHubService, private alert: SnackBarService, private clipboard: Clipboard, private titleNavbarService: TitleNavbarService)
  {}
  ...
  {
    this.subs.add(
      this.studentService.getTransactionsByStudentId(this.studentShareId.currentEnrollment).subscribe((data: any) => {
        this.rowData = data;
        this.applyPagination();
        this.studentFullName = data[0].studentFullName;
        //console.log("Student name: ",data[0].studentFullName)
      })
    );
  }
  ...
  private async copyAllDataTable(): Promise<void> {
    try {
      ...
      const response = await lastValueFrom(this.studentService.getTransactionsByStudentId(this.studentShareId.currentEnrollment));
      ...
      const ignoreKeys = ['paymentId'];
      const orderedKeys = ['descriptionEnglish', 'receivedFrom', 'method', 'amountMT', 'status', 'dateRegister', 'trainerName']; // ordem desejada
  ...
  private async exportExcelAllDataTable(): Promise<void> {
    try {
      ...
      const excelColumns = [
        { header: 'Description', key: 'descriptionEnglish', width: 75 },
        { header: 'Received From', key: 'receivedFrom', width: 30 },
        { header: 'Method', key: 'method', width: 15 },
        { header: 'Amount (MT)', key: 'amountMT', width: 25 },
        { header: 'Status', key: 'status', width: 15 },
        { header: 'Date & Time', key: 'dateRegister', width: 30 },
        { header: 'User', key: 'trainerName', width: 25 },
      ];
      ...
      students.forEach((student, index) => {
        const rowNumber = index + 6; // Starting from row 5 (after header)

        // Set row values
        const row = worksheet.getRow(rowNumber);
        row.values = [
          student.descriptionEnglish ?? '',
          student.receivedFrom ?? '',
          student.method ?? '',
          this.formatAmount(student.amountMT) ?? '',
          student.status ?? '',
      ...
      worksheet.eachRow((row, rowNumber) => {
        if (rowNumber > 5) { // Ignora o cabeçalho (linha 1)
      ...
      worksheet.mergeCells(`A${calc}:G${calc}`);
      footer.height = 20;
      ...
      titleCell2.value = 'Financial Summary – Full List';
      titleCell2.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      titleCell2.alignment = { vertical: 'middle', horizontal: 'center' };

      worksheet.mergeCells('A3:F3');
      const titleCell3 = worksheet.getCell('A3');
      titleCell3.value = this.studentFullName;
      titleCell3.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      ...
      FileSaver.saveAs(blob, 'ETC_financial_summary_full_list.xlsx');
  ...
  private async exportExcelFilteredTable(): Promise<void> {
    try {
      ...
      worksheet.mergeCells('A2:F2');
      const titleCell2 = worksheet.getCell('A2');
      titleCell2.value = 'Financial Summary – Filtered List';
      titleCell2.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      titleCell2.alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      FileSaver.saveAs(blob, 'ETC_financial_summary_filtered_list.xlsx');
  ...
  private async exportPdfAllDataTable(): Promise<void> {
    try {
      ...
      const data = students.map(student => [
          ...
          student.receivedFrom ?? '',
          student.method ?? '',
          { content: this.formatAmount(student.amountMT) ?? ''},
          student.status ?? '',
          student.dateRegister ?? '',
          student.trainerName ?? ''
      ]);
      ...
      doc.text('Financial Summary – Full List', 100, 23);
      ...
      doc.setTextColor(44, 44, 44);
      doc.setFont('helvetica', 'normal');
      doc.text(this.studentFullName+'', 5, 33);
      ...
      // Salva o PDF
      doc.save('ETC_financial_summary_full_list.pdf');
  ...
  private async exportPdfFilteredTable(): Promise<void> {
    try {
      ...
      doc.setTextColor(44, 44, 44);
      doc.setFont('helvetica', 'normal');
      doc.text('Financial Summary – Filtered List', 100, 23);
      ...
      // Salva o PDF
      doc.save('ETC_financial_summary_filtered_list.pdf');
