...
import { DialogMonthlyTuitionTableComponent } from '../dialog-monthly-tuition-table/dialog-monthly-tuition-table.component';

ModuleRegistry.registerModules([ AllCommunityModule]);

@Component({
  selector: 'app-monthy-tuition-main-view',
  imports: [
    AgGridAngular,
    MatIconModule,
    MatTooltipModule,
  ...
  templateUrl: './monthy-tuition-main-view.component.html',
  styleUrl: './monthy-tuition-main-view.component.scss'
})
export class MonthyTuitionMainViewComponent implements OnInit, OnDestroy {
    ...
    [
      {
        ...
        field: 'fullName', minWidth: 260, flex: 1,
        cellClass: 'custom-cell-start'
      },
      {
        headerName: 'Package',
        field: 'package', minWidth: 100, flex: 1,
      ...
      {
        headerName: 'Modality',
        field: 'modality', minWidth: 100, flex: 1,
        cellClass: 'custom-cell-center',
        cellRenderer: (params: any) => {
      ...
      },
      {
        ...
        field: 'amountFormatted', minWidth: 150, flex: 1,
        cellClass: 'custom-cell-end'
      },
      {
        headerName: 'Description',
        field: 'description', minWidth: 210, flex: 1,
        cellClass: 'custom-cell-center'
      },
      {
        headerName: 'Start Date',
        field: 'startDate', minWidth: 130, flex: 1,
        cellClass: 'custom-cell-center'
      },
      {
        headerName: 'Due Date',
        field: 'dueDate', minWidth: 130, flex: 1,
        cellClass: 'custom-cell-center'
      },
      {
        headerName: 'Status',
      ...
      },
      {
        headerName: 'Actions',
        minWidth: 110, flex: 1,
        cellRenderer: DialogMonthlyTuitionTableComponent,
        cellClass: 'custom-cell-center'
      }
    ];
...
// &copy; 2025 | Ramadan I.A. Ismael · License: English Training Center · All rights reserved
  ...
  constructor(private courseInfoService: StudentCourseInfoService, private notificationHub: NotificationHubService, private alert: SnackBarService, private clipboard: Clipboard, private titleNavbarService: TitleNavbarService)
  {}
  ...
  {
    this.subs.add(
      this.courseInfoService.getMonthlyTuitionPaymentList().subscribe((data: any) => {
        this.rowData = data;
  ...
  private async copyAllDataTable(): Promise<void> {
    try {
      ...
      const response = await lastValueFrom(this.courseInfoService.getMonthlyTuitionPaymentList());
      ...
      const ignoreKeys = ['orderMonthlyTuition', 'studentID'];
      const orderedKeys = ['fullName', 'package', 'level', 'modality', 'amountFormatted', 'description', 'startDate', 'dueDate', 'status']; // ordem desejada
  ...
  private async exportExcelAllDataTable(): Promise<void> {
    try {
      ...
      const excelColumns = [
        ...
        { header: 'Amount (MT)', key: 'amountFormatted', width: 20 },
      ...
      titleCell2.value = 'Monthly Tuition – Full List';
      ...
      FileSaver.saveAs(blob, 'ETC_monthly_tuition_full_list_data.xlsx');
  ...
  private async exportExcelFilteredTable(): Promise<void> {
    try {
      ...
      worksheet.mergeCells('A2:G2');
      const titleCell2 = worksheet.getCell('A2');
      titleCell2.value = 'Monthly Tuition – Filtered List';
      titleCell2.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      titleCell2.alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      FileSaver.saveAs(blob, 'ETC_monthly_tuition_filtered_list.xlsx');
  ...
  private async exportPdfAllDataTable(): Promise<void> {
    try {
      ...
      doc.text('Monthly Tuition – Full List', 100, 23);
      ...
      // Salva o PDF
      doc.save('ETC_monthly_tuition_full_list_data.pdf');
  ...
  private async exportPdfFilteredTable(): Promise<void> {
    try {
      ...
      doc.setTextColor(44, 44, 44);
      doc.setFont('helvetica', 'normal');
      doc.text('Monthly Tuition – Filtered List', 100, 23);
      ...
      // Salva o PDF
      doc.save('ETC_monthly_tuition_filtered.pdf');
      ...
      printJS({
        ...
        onError: () => {
         // console.error('PrintJS error:', error);
          ...
          doc.save('ETC_monthyl_tuition_full_list_data.pdf');
        }
      });
    } catch (error) {
      ...
      printJS({
        ...
        onError: () => {
         // console.error('PrintJS error:', error);
          this.alert.show('Oops! Direct printing failed.', 'error');
          // Fallback para download
          doc.save('ETC_monthly_tuition_filtered_data.pdf');
        }
      });
    } catch (error) {
