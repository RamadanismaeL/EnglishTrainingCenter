...
import { BtnStudentFinishedProfileActionTableComponent } from '../../../_components/Students/btn-student-finished-profile-action-table/btn-student-finished-profile-action-table.component';

ModuleRegistry.registerModules([ AllCommunityModule]);

@Component({
  standalone: true,
  selector: 'app-student-finished',
  imports: [
    AgGridAngular,
    MatIconModule,
    MatTooltipModule,
  ...
  templateUrl: './student-finished.component.html',
  styleUrl: './student-finished.component.scss'
})
export class StudentFinishedComponent implements OnInit, OnDestroy {
  ...
  columnDefs: ColDef[] =
    [
      ...
      },
      {
        headerName: 'Profile',
        minWidth: 100, flex: 1,
        cellRenderer: BtnStudentFinishedProfileActionTableComponent,
        cellClass: 'custom-cell-center'
      }
    ];
...
// &copy; 2025 | Ramadan I.A. Ismael · License: English Training Center · All rights reserved
  ...
  private async copyAllDataTable(): Promise<void> {
    try {
      ...
      const response = await lastValueFrom(this.studentService.getListStudentCompleted());
  ...
  private async exportExcelAllDataTable(): Promise<void> {
    try {
      ...
      titleCell2.value = 'Students : Completed – Full List';
      ...
      FileSaver.saveAs(blob, 'ETC_students_completed_full_list_data.xlsx');
  ...
  private async exportExcelFilteredTable(): Promise<void> {
    try {
      ...
      worksheet.mergeCells('A2:G2');
      const titleCell2 = worksheet.getCell('A2');
      titleCell2.value = 'Students : Completed – Filtered List';
      titleCell2.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      titleCell2.alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      FileSaver.saveAs(blob, 'ETC_filtered_students_completed_data.xlsx');
  ...
  private async exportPdfAllDataTable(): Promise<void> {
    try {
      ...
      doc.text('Students : Completed – Full List', 93, 23);
      ...
      // Salva o PDF
      doc.save('ETC_students_completed_full_list_data.pdf');
  ...
  private async exportPdfFilteredTable(): Promise<void> {
    try {
      ...
      doc.setTextColor(44, 44, 44);
      doc.setFont('helvetica', 'normal');
      doc.text('Students : Completed – Filtered List', 88, 23);
      ...
      // Salva o PDF
      doc.save('ETC_student_completed_filtered.pdf');
      ...
      printJS({
        ...
        onError: () => {
         // console.error('PrintJS error:', error);
          this.alert.show('Oops! Direct printing failed.', 'error');
          // Fallback para download
          doc.save('ETC_list_of_student_completed_filtered_data.pdf');
        }
      });
    } catch (error) {
