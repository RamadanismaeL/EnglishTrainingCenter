...
import { BtnStudentDropoutsManageEvaluationsActionTableComponent } from '../../../../_components/Students/btn-student-dropouts-manage-evaluations-action-table/btn-student-dropouts-manage-evaluations-action-table.component';

ModuleRegistry.registerModules([ AllCommunityModule]);

@Component({
  selector: 'app-student-dropouts-manage-evaluations',
  imports: [
    AgGridAngular,
    MatIconModule,
    MatTooltipModule,
  ...
  templateUrl: './student-dropouts-manage-evaluations.component.html',
  styleUrl: './student-dropouts-manage-evaluations.component.scss'
})
export class StudentDropoutsManageEvaluationsComponent implements OnInit, OnDestroy {
  ...
  columnDefs: ColDef[] =
    [
      ...
      },
      {
        headerName: 'Actions',
        minWidth: 110, flex: 1,
        cellRenderer: BtnStudentDropoutsManageEvaluationsActionTableComponent,
        cellClass: 'custom-cell-center'
      }
    ];
  ...
  {
    this.subs.add(
      this.studentCourseInfo.getListStudentCourseInfoInactive().subscribe((data: any) => {
        this.rowData = data;
  ...
  private async copyAllDataTable(): Promise<void> {
    try {
      ...
      const response = await lastValueFrom(this.studentCourseInfo.getListStudentCourseInfoInactive());
  ...
  private async exportExcelAllDataTable(): Promise<void> {
    try {
      ...
      titleCell2.value = 'Students : Inactive / Manage Evaluations – Full List';
      ...
      FileSaver.saveAs(blob, 'ETC_students_inactive_full_list_manage_evaluations_data.xlsx');
  ...
  private async exportExcelFilteredTable(): Promise<void> {
    try {
      ...
      worksheet.mergeCells('A2:F2');
      const titleCell2 = worksheet.getCell('A2');
      titleCell2.value = 'Students : Inactive / Manage Evaluations – Filtered List';
      titleCell2.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      titleCell2.alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      FileSaver.saveAs(blob, 'ETC_filtered_students_inactive_manage_evaluations_data.xlsx');
  ...
  private async exportPdfAllDataTable(): Promise<void> {
    try {
      ...
      doc.text('Students : Inactive / Manage Evaluations – Full List', 60, 23);
      ...
      // Salva o PDF
      doc.save('ETC_students_inactive_manage_evaluations_full_list_data.pdf');
  ...
  private async exportPdfFilteredTable(): Promise<void> {
    try {
      ...
      doc.setTextColor(44, 44, 44);
      doc.setFont('helvetica', 'normal');
      doc.text('Students : Inactive / Manage Evaluations – Filtered List', 50, 23);
      ...
      // Salva o PDF
      doc.save('ETC_students_inactive_manage_evaluations_filtered_list.pdf');
      ...
      printJS({
        ...
        onError: () => {
         // console.error('PrintJS error:', error);
          this.alert.show('Oops! Direct printing failed.', 'error');
          // Fallback para download
          doc.save('ETC_students_inactive_manage_evaluations_filtered_list_data.pdf');
        }
      });
    } catch (error) {
