...
@Component({
  selector: 'app-student-dropouts-manage-status',
  imports: [
    AgGridAngular,
    MatIconModule,
    MatTooltipModule,
  ...
  templateUrl: './student-dropouts-manage-status.component.html',
  styleUrl: './student-dropouts-manage-status.component.scss'
})
export class StudentDropoutsManageStatusComponent implements OnInit, OnDestroy {
  ...
  navigateTo (breadcrumbs: { label: string, url?: any[] }) {
    if (this.getSelectedStudentId().length > 0)
    {
      this.router.navigate(
        ['/',
          { outlets :
            { ramRouter: ['student-dropouts-re-enrollment'] }
          }
        ],
        { state : { idStudentInactive : this.getSelectedStudentId() } } // Passa o idStudentFinished via estado
      );
  ...
  getSelectedStudentOrder(): number[] {
    const selectedNodes = this.gridApi.getSelectedNodes();
  ...
  markCompleted(): void {
    ...
    if (this.getSelectedStudentOrder().length > 0) {
      this.subs.add(
        this.studentService.updateStatus(this.getSelectedStudentOrder(), 'Completed').subscribe({
          next: (response) => {
