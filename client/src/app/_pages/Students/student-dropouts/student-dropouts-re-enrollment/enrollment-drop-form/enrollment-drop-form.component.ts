...
@Component({
  selector: 'app-enrollment-drop-form',
  imports: [
    MatInputModule,
    MatIconModule,
    MatSelectModule,
  ...
  templateUrl: './enrollment-drop-form.component.html',
  styleUrl: './enrollment-drop-form.component.scss'
})
export class EnrollmentDropFormComponent implements OnInit, OnDestroy {
  expirationDate: DateAdapter<Date, any>;
  private readonly fb = inject(FormBuilder);
  private readonly stepperService = inject(StepperReEnrollmentDropService);
  ...
  private idStudentInactive : string = '';
  private orderStudent : number = 0;

  form! : FormGroup;

  constructor (private dateAdapter: DateAdapter<Date>, private studentEditPersonal: StudentEditPersonalDataService, private notificationHub: NotificationHubService, private router: Router, private studentService: StudentsService, private courseInfoReEnrollment: CourseInfoReEnrollmentService)
  {
    ...
    if (navigation?.extras.state) {
      this.idStudentInactive = navigation.extras.state['idStudentInactive'];
    }
  }
  ...
  private initializeForm() : void {
    this.form = this.fb.group({
      ...
      level : ['', [Validators.required, Validators.nullValidator]],
  ...
  {
    this.subs.add(
      this.studentService.GetStudentListProfileEditById(this.idStudentInactive).subscribe({
        next: (response) =>
          {
            this.orderStudent = response.order ?? 0
  ...
  update()
  {
    if (this.form.valid) {
      ...
      ({
        idStudent: this.idStudentInactive,
