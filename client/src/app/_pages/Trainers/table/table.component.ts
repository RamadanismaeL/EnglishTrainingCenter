...
//import { AllEnterpriseModule, LicenseManager, IntegratedChartsModule, ExcelExportModule } from "ag-grid-enterprise";
//import { AgChartsEnterpriseModule } from "ag-charts-enterprise";

import { lastValueFrom } from 'rxjs';

// Excel := npm install exceljs file-saver --save
...
export class TableComponent implements OnInit, OnDestroy {
  ...
  [
    ...
    {
      headerName: 'Email',
      field: 'email', minWidth: 300, flex: 1,
      cellClass: 'custom-cell-start'
    },
    {
      headerName: 'Phone Number',
      field: 'phoneNumber', minWidth: 150, flex: 1,
      cellClass: 'custom-cell-center'
    },
    {
      headerName: 'Position',
    ...
    },
    {
      ...
      valueFormatter: (params) => {
        // Exemplo: converte array de roles em string separada por vírgula
        return Array.isArray(params.value) ? params.value.join(', ') : JSON.stringify(params.value);
      },
      minWidth: 90, flex: 1,
      cellClass: 'custom-cell-center'
    },
    {
      ...
      field: 'dateRegister', minWidth: 180, flex: 1,
      cellClass: 'custom-cell-center'
    },
    {
      field: 'Actions',
      headerName: 'Actions',
      minWidth: 110, flex: 1,
      cellRenderer: BtnTrainerActionsTableComponent,
      cellClass: 'custom-cell-center'
    }
  ];
  ...
  navigateTo (breadcrumbs: { label: string}) {
    this.titleNavbarService.addBreadcrumb(breadcrumbs);
  }

  // Formata Data para exportação (excel, pdf, copy, etc...)
  ...
  {
    this.subs.add(
      this.trainerService.details().subscribe((data: any) => {
        this.rowData = data;
  ...
  private async copyAllDataTable(): Promise<void> {
    try {
      ...
      const response = await lastValueFrom(this.trainerService.details());
      ...
      // 2. Converte para CSV (ou pode usar JSON.stringify() para JSON)
      const headers = Object.keys(trainers[0]).join('\t');
      const csvData = [
        headers,
        ...trainers.map(trainer =>
          Object.values(trainer).join('\t')
        )
      ].join('\n');
      ...
      console.error('Error copying trainer data:', error);
      this.alert.show('Error copying trainer data.', 'error');
    }
  }
  ...
  private async exportExcelAllDataTable(): Promise<void> {
    try {
      ...
      const excelColumns = [
        ...
        { header: 'Roles', key: 'roles', width: 13 },
        { header: 'Date Register', key: 'dateRegister', width: 25 }
      ];
      ...
      worksheet.eachRow((row, rowNumber) => {
        if (rowNumber > 4) { // Ignora o cabeçalho (linha 1)
          ...
          row.eachCell({ includeEmpty: true }, cell => {
            ...
            if (['D', 'F', 'G'].includes(columnLetter)) {
      ...
      titleCell2.value = 'List of All Trainers';
      ...
      FileSaver.saveAs(blob, 'ETC_list_of_trainers_all_data.xlsx');
  ...
  private async exportExcelFilteredTable(): Promise<void> {
    try {
      ...
      worksheet.mergeCells('A2:G2');
      const titleCell2 = worksheet.getCell('A2');
      titleCell2.value = 'Filtered Trainer List';
      titleCell2.font = { size: 20, bold: true, color: { argb: '2C2C2C' } };
      titleCell2.alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      FileSaver.saveAs(blob, 'ETC_list_of_trainers_filtered_data.xlsx');
  ...
  private async exportPdfAllDataTable(): Promise<void> {
    try {
      ...
      doc.text('List of All Trainers', 115, 23);
      ...
      // Salva o PDF
      doc.save('ETC_list_of_trainers_all_data.pdf');
  ...
  private async exportPdfFilteredTable(): Promise<void> {
    try {
      ...
      doc.setTextColor(44, 44, 44);
      doc.setFont('helvetica', 'normal');
      doc.text('Filtered Trainer List', 115, 23);
